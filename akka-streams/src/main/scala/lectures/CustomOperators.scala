...
//  randomGeneratorSource.to(batcherSink).run()
  ...
  /**
    ...
    * 2 ports: an input port, an output port
    */
  class SimpleFilter[T](predicate: T => Boolean)
      extends GraphStage[FlowShape[T, T]] {

    val inPort: Inlet[T] = Inlet[T]("")
    val outPort: Outlet[T] = Outlet[T]("")
  ...
  randomGeneratorSource
    .via(myFilter)
    .to(batcherSink)
    .run() //prints random numbers greater than 50 in batches of 10
  ...
  // .map(x => if (x == 7) throw new RuntimeException("gotcha!") else x)
    ...
    .to(
      Sink.foreach(
        x =>
          if (x == 7) throw new RuntimeException("gotcha, sink!")
          else println(x)
      )
    )
