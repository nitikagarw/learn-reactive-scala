...
object CustomGraphShapes extends App {
  ...
  val balance2x3Graph = RunnableGraph.fromGraph(GraphDSL.create() {
    implicit builder =>
      import GraphDSL.Implicits._
...
//  balance2x3Graph.run()
  ...
  /**
    ...
    */
  case class BalanceMxN[T](override val inlets: List[Inlet[T]],
  ...
  object BalanceMxN {
    def apply[T](inputCount: Int,
                 outputCount: Int): Graph[BalanceMxN[T], NotUsed] =
  ...
  val balanceMxNGraph = RunnableGraph.fromGraph(GraphDSL.create() {
    implicit builder =>
      import GraphDSL.Implicits._
