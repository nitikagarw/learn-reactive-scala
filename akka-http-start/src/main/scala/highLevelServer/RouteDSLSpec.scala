...
class RouteDSLSpec
  ...
  "A digital library backend" should {
    "return all the books in the library" in {
      // Send an HTTP Request through an endpoint
    ...
    "return a book in the library by hitting queryParam endpoint" in {
      Get("/api/book?id=2") ~> libraryRoute ~> check {
        status shouldBe StatusCodes.OK
        responseAs[Option[Book]] shouldBe Some(
          Book(2, "JRR Tolkien", "The Lord of the Rings")
        )
      }
    }

    "return a book by calling the endpoint with ID in path" in {
...
object RouteDSLSpec extends BookJsonProtocol with SprayJsonSupport {
  ...
  /**
    * - GET /api/book => Returns all books in the library
    * - GET /api/book/{id} => Returns a single book with id=X
    * - GET /api/book?id=X => Returns a single book with id=X
    * - POST /api/book => Adds the book to the library
    * - GET /api/book/author/{author} => Returns all the books of the author
    */
